...
Main interfaces:
 - addFrame: stores a new frame in the buffer
 - calcBackground: calculates the temporal median along the frame buffer
 - getAbsDiff: calculates the absolute difference between the last frame, 
...
#define IMW __IMW_keyword__
#define IMH __IMH_keyword__
#define BX __BX_keyword__
#define BY __BY_keyword__
#define nTPB (BX*BY)
#define IMN 21
...
class _FrameBufferNumPy(FrameBufferABC):
    """
    ...
    def __init__(self, image_width=1024, image_height=1, bx=None, by=None, oneD=False, dtype=np.uint16) -> None:
        ...
        self.last_frame = np.zeros(shape=(self.image_width, self.image_height), dtype=self.dtype)
        # create frame buffer
        self.frame_buffer = np.zeros(shape=(self.buffer_length, self.image_width, self.image_height), dtype=self.dtype)
        self._background = np.zeros(shape=(self.image_width, self.image_height), dtype=self.dtype)

    def addFrame(self, frame: np.ndarray, addToBuffer=True):
        """
        ...
        if addToBuffer:
            self._idx = (self._idx+1) % self.buffer_length  # Advance internal frame counter
            self.frame_buffer[self._idx] = frame.reshape((self.image_width, self.image_height))  # TODO: avoid this reshape operation?
        else:
            self.last_frame = frame

    def calcBackground(self):

        self._background = np.median(self.frame_buffer, axis=0).astype(self.frame_buffer.dtype)
...
class _FrameBufferCUDA(FrameBufferABC):
    """
    ...
    def __init__(self, image_width=1024, image_height=1, bx=32, by=1, oneD=False, dtype=np.uint16) -> None:
        ...
        self.image_width = int(image_width)  # force to int to avoid parameters as numpy.int64
        ...
        self.frame_buffer = np.zeros(shape=(self.buffer_length, self.image_width, self.image_height), dtype=self.dtype)
        self._background = np.zeros(shape=(self.image_width, self.image_height), dtype=self.dtype)
        self._absDiff = np.zeros(shape=(self.image_width, self.image_height), dtype=self.dtype)
    ...
    def addFrame(self, frame: np.ndarray, addToBuffer=True):
        ...
        if addToBuffer:
            self._idx = (self._idx+1) % self.buffer_length  # Advance internal frame counter
            buffer_offset = int((self._idx % self.buffer_length) * self.frame_size_bytes)
...
if use_cuda:
    FrameBuffer = _FrameBufferCUDA
else:
    FrameBuffer = _FrameBufferNumPy


if __name__ == '__main__':
    # Run some performance benchmarks, comparing the two implementations

    print(f'RotatingFrameBuffer - using CUDA: {use_cuda}')
    ...
    buff1 = FrameBuffer(im_x, im_y, bx=32, by=16, dtype=dtype)
    ...
    all_frames = np.random.random_integers(0, 255*255, size=(total_cycles, buff1.image_width, buff1.image_height)).astype(buff1.dtype)

    for i in range(1,total_cycles+1):

        # Add frame to buffer
        this_frame = all_frames[i-1]
        buff1.addFrame(this_frame, addToBuffer=i%2)

        # Update the background using the median calculation
        buff1.calcBackground()

        # Calculate the absolute difference between the current 
        # frame and the new background, and get the result back
        absDiff1 = buff1.getAbsDiff()
    ...
    if use_cuda:
        # Repeat, without CUDA
        print(f'RotatingFrameBuffer - without CUDA')

        buff2 = _FrameBufferNumPy(im_x, im_y, bx=32, by=16, dtype=dtype)
        t1 = time.time()
        ...
        for i in range(1,total_cycles+1):
            ...
            # Add frame to buffer
            this_frame = all_frames[i-1]
            buff2.addFrame(this_frame, addToBuffer=i%2)

            # Update the background using the median calculation
            buff2.calcBackground()

            # Calculate the absolute difference between the current 
            # frame and the new background, and get the result back
            absDiff2 = buff2.getAbsDiff()
